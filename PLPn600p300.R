...
r3 = 1.15
adj = 0.1
int = TRUE #Boolean value, whether to return Bernstein Poly with intercept
...
#control sequence of Z1,Z2,Z3,Z4
Z1.c <- seq(l1+adj,r1-adj,by=0.001) 
Z2.c <- seq(l2+adj,r2-adj,by=0.001) 
Z3.c <- seq(l2+adj,r2-adj,by=0.001)
Z4.c <- seq(l3+adj,r3-adj,by=0.001)
...
nonlin.estBARBIC1 <- matrix(0,B,length(Z1.c)) 
...
while(a <= B){
  ...
  Z4_t <- sapply(1:n, FUN=function(i) 0.1*(Z4[i]+1)^3)
  
  Y <- rpois(n, lambda = exp(X%*%beta + W%*%alpha + Z1_t + Z2_t + Z3_t + Z4_t))
  
  cat("The number of zeros and max is", length(Y[Y==0]),"and",max(Y), "\n")
  ...
  cData <- createCyclopsData(Y ~ Xmat, modelType = "pr")
  ...
  FitBARAIC <- try(fitCyclopsModel(cData, prior=PrAIC))
  FitBARBIC <- try(fitCyclopsModel(cData, prior=PrBIC))

  #Initial values of the coefficients is done such that convergence is achieved
  cvRidge <- cv.glmnet(x=Xmat, y=Y, family="poisson", type.measure="deviance", alpha=0)
  cvLasso <- cv.glmnet(x=Xmat, y=Y, family="poisson", type.measure="deviance", alpha=1, 
                        penalty.factor=c(rep(1,p),rep(0,qW+qZ*(b+as.integer(int)))))
  FitLasso <- glmnet(x=Xmat, y=Y, family="poisson", alpha=1, lambda=quantile(cvLasso$lambda,0.8),
                     penalty.factor=c(rep(1,p),rep(0,qW+qZ*(b+as.integer(int)))))
  coefRidge <- coef(cvRidge, s=cvRidge$lambda.min)[-1]
  cvAlasso <- cv.glmnet(x=Xmat, y=Y, family="poisson", type.measure="deviance", alpha = 1, 
                        penalty.factor =  c(1/abs(coefRidge[1:p]), rep(0,qW+qZ*(b+as.integer(int)))) )
  Alassofit <- glmnet(x=Xmat, y=Y, family ="poisson", alpha = 1, lambda = quantile(cvAlasso$lambda,0.7),
                      penalty.factor = c(1/abs(coefRidge[1:p]), rep(0,qW+qZ*(b+as.integer(int)))))
  Xmat_red <- Xmat[,-zpos]
  GLMest <- coef(glm(Y ~ Xmat_red, family = "poisson"))[(2:(pz+qW+1))]
...
write(x=paste("TP of BAR-AIC is :", m_TP1, sep = ""),fileConn, append = TRUE)
write(x=paste("TP of Lasso is :", m_TP2, sep = ""), fileConn, append = TRUE)
write(x=paste("TP of ALasso is :", m_TP3, sep = ""), fileConn, append = TRUE)
write(x=paste("TP of BAR-BIC is :", m_TP4, sep = ""), fileConn, append = TRUE)
write(x=paste("FP of BAR-AIC is :", m_FP1, sep = ""), fileConn, append = TRUE)
write(x=paste("FP of Lasso is :", m_FP2, sep = ""), fileConn, append = TRUE)
write(x=paste("FP of ALasso is :", m_FP3, sep = ""), fileConn, append = TRUE)
write(x=paste("FP of BAR-BIC is :", m_FP4, sep = ""), fileConn, append = TRUE)
...
write(x=paste("Bias of Betas for ALasso:", round(b_ALasso,3), sep=""), fileConn2, append=T)
write(x=paste("Bias of Betas for Oracle:", round(b_Oracle,3), sep=""), fileConn2, append=T)
write(x=paste("Bias of Alphas for BAR-AIC:", round(alphab_BARAIC,3), sep=""), fileConn2, append=T)
write(x=paste("Bias of Alphas for BAR-BIC:", round(alphab_BARBIC,3), sep=""), fileConn2, append=T)
write(x=paste("Bias of Alphas for Lasso:", round(alphab_Lasso,3), sep=""), fileConn2, append=T)
write(x=paste("Bias of Alphas for ALasso:", round(alphab_ALasso,3), sep=""), fileConn2, append=T)
write(x=paste("Bias of Alphas for Oracle:", round(alphab_Oracle,3), sep=""), fileConn2, append=T)
write(x=paste("SD of Betas for BAR-AIC:", round(sdBARAIC,3), sep=""), fileConn2, append=T)
write(x=paste("SD of Betas for BAR-BIC:", round(sdBARBIC,3), sep=""), fileConn2, append=T)
write(x=paste("SD of Betas for Lasso:", round(sdLasso,3), sep=""), fileConn2, append=T)
write(x=paste("SD of Betas for ALasso:", round(sdALasso,3), sep=""), fileConn2, append=T)
write(x=paste("SD of Betas for Oracle:", round(sdOracle,3), sep=""), fileConn2, append=T)
write(x=paste("SD of Alphas for BAR-AIC:", round(sdqwBARAIC,3), sep=""), fileConn2, append=T)
write(x=paste("SD of Alphas for BAR-BIC:", round(sdqwBARBIC,3), sep=""), fileConn2, append=T)
write(x=paste("SD of Alphas for Lasso:", round(sdqwLasso,3), sep=""), fileConn2, append=T)
write(x=paste("SD of Alphas for ALasso:", round(sdqwALasso,3), sep=""), fileConn2, append=T)
write(x=paste("SD of Alphas for Oracle:", round(sdqWOracle,3), sep=""), fileConn2, append=T)

psiZ1true <- 0.1*(Z1.c-3)^2
psiz2true <- 0.2*(cos(2*pi*Z2.c)+1)
psiz3true <- 0.2*sin(2*pi*Z3.c)
psiz4true <- 0.1*(Z4.c+1)^3 
